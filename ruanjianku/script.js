...
let allSelected = false; // 用于跟踪是否所有库已被全选
...
selectAllBtn.addEventListener('click', () => {
    const deleteCheckboxes = document.querySelectorAll('.delete-checkbox');
    allSelected = !allSelected; // 切换全选/取消全选状态

    deleteCheckboxes.forEach(checkbox => {
        checkbox.checked = allSelected; // 根据状态进行选择/取消选择
        checkbox.style.display = 'inline-block'; // 确保复选框显示
    });

    // 切换按钮文本
    selectAllBtn.textContent = allSelected ? "取消全选库" : "全选软件库";
});
